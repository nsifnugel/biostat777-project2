...
percent_union <- demographics %>%
  ...
  summarize(mean_members = mean(p_members),
            ...
            CI_lowerbound_members = calculate_CI(biostat777project2nsifnugel:::make_ci_class(p_members))[1],
            CI_upperbound_members = calculate_CI(biostat777project2nsifnugel:::make_ci_class(p_members))[2],
            mean_covered = mean(p_covered),
            sd_covered = sd(p_covered), 
            CI_lowerbound_covered = calculate_CI(biostat777project2nsifnugel:::make_ci_class(p_covered))[1],
            CI_upperbound_covered = calculate_CI(biostat777project2nsifnugel:::make_ci_class(p_covered))[2]) %>%
